...
## Linear Regression: Device year as a predictor of Step Count MAPE + Wear location control
...
clean_data$lm_year_MAPE_loc <- lm_year_MAPE_loc$fitted.values

scatter_fitted_year <- ggplot(data = predicted_data, aes(x = device_year_c, y = MAPE)) +
      geom_point(alpha = 0.2) +
      stat_smooth(method = "lm", colour = "gray") +
      theme_bw()
...
```{r}
lmer_year_MAPE <- lmer(MAPE ~ device_year_c, data = clean_data)
...
lmer_year_by_brand_MAPE <- lm(MAPE ~ device_year*Brand, data = clean_data)

summary(lmer_year_by_brand_MAPE)
lmer_year_by_brand_MAPE_table <- tbl_regression(lmer_year_by_brand_MAPE)
lmer_year_by_brand_MAPE_table %>% as_kable()
...
```{r}
lm_year_MAPE_fb <- lm(MAPE ~ device_year_c, data = clean_data_fitbit)
...
```{r}
lm_year_MAPE_loc_fb <- lm(MAPE ~ device_year_c + Wear_Location, data = clean_data_fitbit)

summary(lm_year_MAPE_loc_fb)
```
